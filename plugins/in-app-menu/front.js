...
module.exports = () => {
	...
	document.addEventListener(
		...
		() => {
			...
			playPageObserver.observe($("ytmusic-app-layout"), {
				attributeFilter: ["player-page-open_", "playerPageOpen_"],
			});
...
function setupSearchOpenObserver() {
	const searchOpenObserver = new MutationObserver((mutations) => {
		$("#nav-bar-background").style.webkitAppRegion = mutations[0].target.opened
			? "no-drag"
			: "drag";
	});
	searchOpenObserver.observe($("ytmusic-search-box"), {
...
function setupMenuOpenObserver() {
	...
	menuOpenObserver.observe($(".cet-menubar"), {
		subtree: true,
...
function setNavbarMargin() {
	$("#nav-bar-background").style.right = $("ytmusic-app-layout").playerPageOpen_
		? "0px"
		: "12px";
}
