...
const createToken = async ({ api_key, api_root, secret }) => {
    ...
    let response = await fetch(
        `${api_root}${createQueryString(data, api_sig)}`,
    );
...
const getAndSetSessionKey = async (config) => {
    ...
    let res = await fetch(
        `${config.api_root}${createQueryString(data, api_sig)}`,
    );
    res = await res.json();
    if (res.error) await authenticate(config);
...
const postSongDataToAPI = async (songInfo, config, data) => {
    ...
    if (!config.session_key) await getAndSetSessionKey(config);
    ...
    registerCallback((songInfo) => {
        ...
        if (!songInfo.isPaused) {
            ...
            if (scrobbleTime > songInfo.elapsedSeconds) {
                ...
                const timeToWait =
                    (scrobbleTime - songInfo.elapsedSeconds) * 1000;
