...
module.exports = (win) => {
    ...
    ipcMain.on('apiLoaded', () =>
        win.webContents.send('setupTimeChangedListener'),
    );
...
const display = (kind) => {
    if (config.get('toastStyle') === ToastStyles.legacy) {
        ...
        return `\
            content="${
                config.get('hideButtonText')
                    ? ''
                    : kind.charAt(0).toUpperCase() + kind.slice(1)
            }"\
            imageUri="file:///${path.resolve(
                __dirname,
                iconLocation,
                `${kind}.png`,
            )}"
...
const xml_image = ({ title, artist, isPaused }, imgSrc, placement) =>
    toast(
        `\
...
const xml_logo = (songInfo, imgSrc) =>
    xml_image(songInfo, imgSrc, 'placement="appLogoOverride"');

const xml_hero = (songInfo, imgSrc) =>
    xml_image(songInfo, imgSrc, 'placement="hero"');

const xml_banner_bottom = (songInfo, imgSrc) => xml_image(songInfo, imgSrc, '');

const xml_banner_top_custom = (songInfo, imgSrc) =>
    toast(
        `\
...
const xml_banner_centered_bottom = ({ title, artist, isPaused }, imgSrc) =>
    toast(
        `\
            <text>ㅤ</text>
            <group>
...
const xml_banner_centered_top = ({ title, artist, isPaused }, imgSrc) =>
    toast(
        `\
            <image id="1" src="${imgSrc}" name="Image" />
            <text>ㅤ</text>
