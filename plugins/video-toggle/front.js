...
const moveVolumeHud = isEnabled('precise-volume')
    ? require('../precise-volume/front').moveVolumeHud
    : () => {};
...
function changeDisplay(showVideo) {
    ...
    $('#song-video.ytmusic-player').style.display = showVideo
        ? 'block'
        : 'none';
...
function forcePlaybackMode() {
    const playbackModeObserver = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (
                mutation.target.getAttribute('playback-mode') !==
                'ATV_PREFERRED'
            ) {
...
function observeThumbnail() {
    ...
    playbackModeObserver.observe($('#song-image img'), {
        attributeFilter: ['src'],
    });
}
...
function forceThumbnail(img) {
    const thumbnails =
        $('#movie_player').getPlayerResponse()?.videoDetails?.thumbnail
            ?.thumbnails;
