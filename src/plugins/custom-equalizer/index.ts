...
export default createPlugin({
  name: () => t('plugins.custom-equalizer.name'),
  description: () => t('plugins.custom-equalizer.description'),
  restartNeeded: false,
  config: {
    enabled: false,
  ...
  menu: async ({ getConfig, setConfig }) => {
    ...
    return [
      {
        label: t('plugins.custom-equalizer.menu.presets.label'),
        submenu: allPresetsD.map((preset) => ({
          restartNeeded: true,
          label: config.customPresets.includes(preset)
            ? preset.name
            : t(`plugins.custom-equalizer.menu.presets.list.${preset.name}`),
          type: 'radio',
      ...
      {
        label: t('plugins.custom-equalizer.menu.reset-custom-presets'),
        toolTip: t(
          'plugins.custom-equalizer.menu.reset-custom-presets-tooltip',
        ),
        click() {
      ...
      {
        label: t('plugins.custom-equalizer.menu.edit-config'),
        toolTip: t('plugins.custom-equalizer.menu.edit-config-tooltip'),
        click() {
          appConfig.edit();
        },
      },
